...
num_reviewing <- sum(as.numeric(lapply(screen.data$First.contact, function(x) sum(!is.na(x)))))
...
We have pre-screened a total of `r dim(screen.data)[1]` candidates who have actively expressed interest (see Table 1 for overall demographics). `r num_prescreen_fail` have been deemed ineligible from pre-screening (i.e., prior to enrollment, see Table 2 below for details) and `r num_lost` have declined or are lost to follow up (after multiple attempts to contact). `r sum(num_reviewing)` have confirmed strong interest following study staff review of study overview and are reviewing the ICF/other recruitment materials.
...
`r num_active` remain in the active candidate screening pool (see Table 4 below for details), `r num_fresh_contacts` of whom are new contacts in the past two weeks. 
...
#### Table 5. Summary data active pool
```{r echo=FALSE, message=FALSE, warning=FALSE}
